...
enum IQOSProtocol {
    ...
    static let chargeStartVibrationOnCommands: [[UInt8]] = [
    ...
    static let chargeStartVibrationOffCommands: [[UInt8]] = [
...
extension IQOSProtocol {
    ...
    static func vibrationCommands(for settings: IQOSVibrationSettings, model: IQOSDeviceModel) throws -> [[UInt8]] {
        if !model.supports(.vibration) {
            throw IQOSError.unsupported("vibration is not supported for model \(model.rawValue)")
        }
        if !model.supports(.chargeStartVibration), settings.whenChargingStart != nil {
            throw IQOSError.unsupported("charge-start vibration is not supported for model \(model.rawValue)")
        }
        if model.supports(.chargeStartVibration), settings.whenChargingStart == nil {
            throw IQOSError.protocolDecode("charge-start vibration value is required for model \(model.rawValue)")
        }

        var commands = [mainVibrationCommand(for: settings)]
        if model.supports(.chargeStartVibration), let chargeStartEnabled = settings.whenChargingStart {
            if chargeStartEnabled || !settings.coreVibrationSettingsDisabled {
                commands.append(contentsOf: chargeStartEnabled ? chargeStartVibrationOnCommands : chargeStartVibrationOffCommands)
            }
        }
        return commands
    }

    private static func mainVibrationCommand(for settings: IQOSVibrationSettings) -> [UInt8] {
        var register: UInt16 = 0
        if settings.whenHeatingStart { register |= 0x0100 }
        if settings.whenStartingToUse { register |= 0x1000 }
        if settings.whenPuffEnd { register |= 0x0001 }
        if settings.whenManuallyTerminated { register |= 0x0010 }
        ...
        return [
            ...
            0x00,
            UInt8(register >> 8),
            UInt8(register & 0x00FF),
            vibrationChecksum(register)
        ]
    }

    private static func vibrationChecksum(_ register: UInt16) -> UInt8 {
        var checksum: UInt8 = 0x77
        if (register & 0x0001) != 0 { checksum ^= 0x07 }
        if (register & 0x0010) != 0 { checksum ^= 0x70 }
        if (register & 0x0100) != 0 { checksum ^= 0x15 }
        if (register & 0x1000) != 0 { checksum ^= 0x57 }
        return checksum
    }
...
private extension IQOSVibrationSettings {
    var coreVibrationSettingsDisabled: Bool {
        !whenHeatingStart && !whenStartingToUse && !whenPuffEnd && !whenManuallyTerminated
    }
}
